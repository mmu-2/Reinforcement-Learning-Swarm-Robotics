...
if load_checkpoint == True:
    ...
    file = open("./dqn_training_data/window", "rb")
    window = pickle.load(file)
    file.close()
    file = open("./dqn_training_data/data", "rb")
    [average_reward, i_episode] = pickle.load(file)
    file.close()
    #graph-related data
    file = open("./dqn_training_data/rewards", "rb")
    y_rewards = pickle.load(file)
    file.close()
    file = open("./dqn_training_data/last100average", "rb")
    y_100 = pickle.load(file)
    file.close()
    file = open("./dqn_training_data/running_average", "rb")
    y_average = pickle.load(file)
    file.close()
    file = open("./dqn_training_data/episodes", "rb")
    x_axis = pickle.load(file)
    file.close()
    # file = open("./dqn_training_data/dqn_agents", "rb")
...
while last_100_average < 300:
    ...
    while done == False:
        ...
        for i in range(len(agents)):
            agents[i].observe(last_observations[i], actions[i], rewards[i], observations[i], done, i_episode)
        ...
        if done:
            ...
            if i_episode % 1 == 0:
                print("Episode {}: average: {:.3f} last 100: {:.3f} current reward: {:.3f}".format(i_episode,
                                                                                                   average_reward,
                                                                                                   last_100_average,
                                                                                                   episode_reward))
                for i in range(len(agents)):
                ...
                # additional checkpoint data so I can reload and start from a later checkpoint
                file = open("./dqn_training_data/window", "wb")
                pickle.dump(window, file)
                file.close()
                data = [average_reward, i_episode]
                file = open("./dqn_training_data/data", "wb")
                pickle.dump(data, file)
                file.close()
                # graph-related data
                file = open("./dqn_training_data/rewards", "wb")
                pickle.dump(y_rewards, file)
                file.close()
                file = open("./dqn_training_data/last100average", "wb")
                pickle.dump(y_100, file)
                file.close()
                file = open("./dqn_training_data/running_average", "wb")
                pickle.dump(y_average, file)
                file.close()
                file = open("./dqn_training_data/episodes", "wb")
                pickle.dump(x_axis, file)
                file.close()
                # file = open("./dqn_training_data/dqn_agents", "wb")
...
for i in range(len(agents)):
    agents[i].save('dqn_agent{}-checkpoint{}'.format(i + 1, i_episode))
env.close()
